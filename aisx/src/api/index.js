...
let service = axios.create({
  timeout: 25000,//超时时间
  baseURL: 'http://127.0.0.1:5000/api', // 设置访问哪个后端
  // 在发送请求之前做些什么
})
...
service.interceptors.request.use(
  function (config) {
    ...
    //   config.headers['Authorization'] = 'Bearer ' + useUserStroe().refresh_token

    // } else {
    //   config.headers['Authorization'] = 'Bearer ' + useUserStroe().access_token
    // }
    // console.log(useUserStroe().access_token)
    return config;
  },
  function (error) {
    // 对请求错误做些什么
...
service.interceptors.response.use(
  function (response) {
    ...
    if (data.code == 200) { // login 登录成功

      response.data = data;
      return Promise.resolve(response);
    }
    else if(data.code == 239) // register
    {
      response.data = data;
      return Promise.resolve(response);
    }
